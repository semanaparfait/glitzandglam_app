...
import { useCartItems, useRemoveFromCart, useUpdateCartItemQuantity } from "@/hooks/cart/useCart";
      ...
      {items.length > 0 ? (
        ...
        <ScrollView className="flex-1 px-4 mt-4" showsVerticalScrollIndicator={false}>
          {items.map((item: any) => (
            // <View  className="p-4 border-b border-gray-200">
              <View key={item.id} className="flex-row mb-4 bg-white p-3 rounded-xl">
                <View className="w-20 h-20 rounded-lg overflow-hidden mr-3 bg-gray-100">
                <Image source={{ uri: item.product.image.replace("http://", "https://") }} className="w-full h-full "  resizeMode='cover'/>
                </View>
                ...
                <View className='flex-1 justify-between'>
                  <View className="flex-row justify-between items-start">
                    ...
                    <TouchableOpacity activeOpacity={0.6} className="p-2" onPress={() => handleRemove(item.id)}>
                      <Ionicons name="trash" size={20} color="#FF4C3B" />
                    </TouchableOpacity>
                  </View>

                  <View className="flex-row justify-between items-center mt-2">
                     <Text className="font-bold text-base">{item.totalPriceFormatted}</Text>
                     <View className="flex-row items-center bg-gray-200 rounded-full px-2 py-1">
                      <TouchableOpacity className="p-1" onPress={() => reduceQuantity(item)}>
                      ...
                      <Text className="mx-2 text-sm">{item.quantity}</Text>
                       <TouchableOpacity
                         ...
                         style={item.quantity >= item.product.stockquantity ? { opacity: 0.4 } : {}}
                       >
        ...
        <View className="p-4 bg-white rounded-t-3xl shadow-sm">
          <View className="flex-row justify-between mb-4">
            ...
            <Text className="text-2xl font-bold">{cartItems?.totalPriceFormatted}</Text>
          </View>
